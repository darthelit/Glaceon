      ...
      <div id={`move-dropdown`} className='dropdown' style={{ verticalAlign: 'middle' }}>
        <div className='dropdown-trigger'>
          <button
            ...
            onClick={() => document.getElementById(`move-dropdown`).classList.toggle('is-active')}
          >
            <span>Moves</span>
        ...
        <div className='dropdown-menu' id={`move-dropdown-menu`} role='menu'>
          <div className='dropdown-content dropdown-scroll'>{this.generateOptions(moves)}</div>
        </div>
      </div>
    );
  }
            ...
            {_filter(this.state.currentPokemon.species.flavor_text_entries, ['language.name', 'en']).map((entry) => (
              <div className='card'>
                <div className='card-content'>
                  <p className='title'>{entry.flavor_text}</p>
                  <p className='subtitle'>{entry.version.name}</p>
                </div>
              </div>
            ))}
          </div>
